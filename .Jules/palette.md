...
## 2025-05-15 - Real-time Input Feedback and Threshold Indicators
**Learning:** Textareas with character limits benefit significantly from real-time counters linked via 'aria-describedby'. Using a 90% threshold for visual warnings (e.g., changing color to 'text-accent') provides a non-intrusive yet clear warning before the user hits the hard limit.
**Action:** Always include character counters for length-limited textareas, ensure they update on 'input' and 'reset' events, and use 'aria-live="polite"' for accessibility.
