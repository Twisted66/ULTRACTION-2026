...
## 2024-05-24 - Functional Decorative Elements and Sticky Header Offset
**Learning:** Decorative elements that imply functionality (like "Scroll" indicators) should always be implemented as accessible anchor links (`<a>`). When using sticky headers, target sections must use `scroll-mt` classes matching the header's height (e.g., `64px`, `72px`, `104px` in this project) to prevent the target content from being obscured by the header after clicking. Additionally, complex `inert` logic for mobile menus must explicitly exclude the header and its ancestor containers (e.g., using `.contains(siteHeader)`) to ensure the menu remains interactive for all users and automated tools.
**Action:** Convert all directional visual indicators to functional links with proper `aria-label` and `scroll-mt` offsets that match the project's header height tokens. Always verify `inert` attribute logic against the entire header tree to prevent UI lockout.
