...
## 2026-02-14 - Real-time Character Constraints and Accessibility
**Learning:** Textareas with character limits should feature a real-time counter linked via `aria-describedby` that provides visual feedback (e.g., using brand accent colors) as the limit approaches. This improves accessibility by informing screen readers and provides a better experience for sighted users.
**Action:** Implement real-time character counters with `aria-live="polite"` and visual thresholds for all text inputs with a significant character limit.
