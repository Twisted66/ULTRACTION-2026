...
## 2026-02-14 - Brand Consistency and Intentional Motion
**Learning:** Hardcoded hex values for brand colors (like `#800000`) bypass the Tailwind theme, making design maintenance difficult and inconsistent across pages. High-impact CTAs in the hero section benefit from subtle motion (e.g., MagneticButton) to draw attention, but must remain keyboard accessible and use semantic theme tokens.
**Action:** Audit and replace hardcoded brand colors with designated Tailwind theme tokens (`accent`, `primary`) and prioritize 'MagneticButton' for primary landing page actions to enhance interactive delight while maintaining accessibility.
