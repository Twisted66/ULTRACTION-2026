...
## 2026-02-14 - Functional and Accessible Scroll Indicators
**Learning:** Purely decorative scroll indicators (like bouncing arrows) create a false expectation of interactivity for mouse users and are completely invisible to keyboard/screen-reader users. Converting these into functional anchor links with `aria-label`, `focus-visible` styles, and `scroll-mt` (to handle sticky headers) transforms a visual flourish into a useful navigation tool.
**Action:** Always ensure visual scroll cues are implemented as semantic links with proper offsets for sticky headers.
